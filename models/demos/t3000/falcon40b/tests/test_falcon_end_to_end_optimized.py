...
from models.demos.t3000.falcon40b.tt.falcon_causallm_optimized import TtFalconCausalLM
...
def run_test_FalconCausalLM_end_to_end(
    ...
    for i in range(num_loops - 1):
        ...
        if llm_mode == "prefill":
            ...
            tt_outs = [
                ttnn.to_torch(tt_out, mesh_composer=ConcatMeshToTensor(device_mesh, dim=-1)) for tt_out in tt_outs
            ]
            tt_out = tt_outs
    ...
    if llm_mode == "prefill":
        ...
        print(tt_outs[0].shape)
        tensors = [ttnn.to_torch(tt_out, mesh_composer=ConcatMeshToTensor(device_mesh, dim=-1)) for tt_out in tt_outs]
        tt_out = torch.vstack(tensors)

    elif llm_mode == "decode":
        print(f"pytorch: shape: {pytorch_out.shape}")
        print(f"tt: shape: {tt_out.shape}")
        tt_out = ttnn.to_torch(tt_out, mesh_composer=ConcatMeshToTensor(device_mesh, dim=-1))
        print(f"tt: shape: {tt_out.shape}")
