...
def trocr_attention(
    ...
    *,
    ...
    query_states = torch.reshape(ttnn.to_torch(query_states), (bsz * num_heads, -1, head_dim))
    query_states = ttnn.from_torch(query_states, device=device, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
    key_states = torch.reshape(ttnn.to_torch(key_states), (bsz * num_heads, -1, head_dim))
    key_states = ttnn.from_torch(key_states, device=device, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
    value_states = torch.reshape(ttnn.to_torch(value_states), (bsz * num_heads, -1, head_dim))
    value_states = ttnn.from_torch(value_states, device=device, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
    ...
    if attn_weights.shape != (bsz * num_heads, tgt_len, src_len):
        raise ValueError(
    ...
    if layer_head_mask is not None:
        if layer_head_mask.shape != (num_heads,):
            raise ValueError(
                f"Head mask for a single layer should be of size {(num_heads,)}, but is" f" {layer_head_mask.shape}"
            )
        attn_weights = ttnn.reshape(attn_weights, (bsz, num_heads, tgt_len, src_len))
    ...
    if attn_output.shape != (bsz * num_heads, tgt_len, head_dim):
        raise ValueError(
            f"`attn_output` should be of size {(bsz, num_heads, tgt_len, head_dim)}, but is" f" {attn_output.shape}"
        )
