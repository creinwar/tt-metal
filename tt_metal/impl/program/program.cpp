...
void Program::finalize_rt_args() {
    ...
    vector<uint32_t> max_rtas(DISPATCH_CLASS_MAX_PROC);
    vector<uint32_t> max_crtas(DISPATCH_CLASS_MAX_PROC);

    for (CoreType core_type : core_types) {
        uint32_t unique_rta_size = 0;

        for (auto& kg : this->get_kernel_groups(core_type)) {
            for (int dispatch_class = 0; dispatch_class < DISPATCH_CLASS_MAX_PROC; dispatch_class++) {
