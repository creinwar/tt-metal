...
static constexpr uint32_t NUM_CHANNELS = get_cmpile_time_arg<uint32_t>(0);

template <bool MEASURE>
FORCE_INLINE void run_loop_interation(
    std::array<uint32_t, NUM_CHANNELS> const& channel_addrs,
    std::array<uint32_t, NUM_CHANNELS> const& channel_sync_addrs,
    uint32_t full_payload_size,
    uint32_t full_payload_size_eth_words
) {
    DPRINT << "S RLI\n";
    if constexpr (MEASURE) {
        DeviceZoneScopedN("LOOP-ITER");
        {
...
void kernel_main() {
    ...
    std::array<uint32_t, NUM_CHANNELS> channel_sync_addrs;
    {
        ...
        for (uint8_t i = 0; i < NUM_CHANNELS; i++) {
            ...
            channel_sync_addrs[i] = channel_addr;
